...
- **GitHub Actions**: CI/CD 자동화 파이프라인을 구축하여 코드 변경 시 테스트, 빌드, 배포 작업을 자동으로 수행.
- **AWS EC2**: 애플리케이션을 호스팅하고 서버를 관리하며, 확장성과 안정성을 제공.
- **AWS CodeDeploy**: 애플리케이션 배포를 자동화하여 효율적이고 안정적인 배포 환경을 구축.
- **AWS ECR (Elastic Container Registry):** Docker 이미지를 저장하고 관리할 수 있는 컨테이너 레지스트리로, CI/CD 파이프라인에서 이미지 배포에 활용.
- **GitHub**: 코드 버전 관리를 위한 플랫폼으로, 협업과 코드 리뷰를 지원.
- **Docker**: 애플리케이션을 컨테이너화하여 이식성과 일관성을 보장하며, CI/CD 및 배포 과정의 효율성을 높임.
- **Notion**: 프로젝트 관리와 협업을 위한 도구로, 작업 현황 및 문서화를 체계적으로 관리.
